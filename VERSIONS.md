...
## v1.7.18
- **Disabled το auto-tap για fullscreen** (rotation): ούτε το JS click ούτε το native
  synthetic touch (με σωστό source flag) κατάφεραν να ενεργοποιήσουν πραγματικά fullscreen
  σε καμία δοκιμή, και το synthetic touch μερικές φορές "έπεφτε" πάνω στο ίδιο το βίντεο
  αντί στο κουμπί, προκαλώντας ανεπιθύμητη παύση — ενεργή παλινδρόμηση, όχι απλά αναποτελε-
  σματικό. Η λειτουργία πλέον μόνο καταγράφει τι θα είχε κάνει, χωρίς να ενεργεί· ο χρήστης
  συνεχίζει να πατάει το πραγματικό κουμπί fullscreen μόνος του.
- **Διερεύνηση για "η εφαρμογή έκλεισε κατά τη διάρκεια κλήσης"**: προστέθηκε (α) global
  crash handler που αποθηκεύει το stack trace σε SharedPreferences (επιβιώνει ακόμα κι αν η
  ίδια η διαδικασία τερματιστεί, σε αντίθεση με το in-memory debug log) και το εμφανίζει στο
  επόμενο άνοιγμα της εφαρμογής, (β) το foreground service που κρατάει ζωντανή τη διαδικασία
  πλέον περιμένει ~45" πριν σταματήσει πραγματικά μετά από παύση (αντί να σταματάει αμέσως) —
  μια κλήση που παυσάρει προσωρινά το βίντεο δεν αφαιρεί πια αμέσως την προστασία ακριβώς τη
  στιγμή που η εφαρμογή είναι πιο ευάλωτη σε τερματισμό από το σύστημα λόγω μνήμης.
