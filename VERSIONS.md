...
## v1.7.11
- **Fix: πολλές φορές το auto-fullscreen σε rotation δεν ενεργοποιούνταν καθόλου** (η οθόνη
  απλά φαρδαίνει σε landscape χωρίς να μπαίνει σε πραγματικό fullscreen — status bar/header
  παραμένουν ορατά). Ο χρήστης παρατήρησε ότι έπρεπε πρώτα να πατήσει μέσα στο βίντεο.
  Αιτία: το controls overlay του player (μαζί με το κουμπί fullscreen) πιθανόν φτιάχνεται
  lazy, μόνο μετά από αλληλεπίδραση — ακριβώς τη στιγμή της περιστροφής μπορεί να μην υπάρχει
  ακόμα στο DOM, οπότε το `querySelector` δεν έβρισκε τίποτα. Fix: τώρα ξαναδοκιμάζει κάθε
  250ms για ~2 δευτερόλεπτα πριν τα παρατήσει (και μετά πέφτει στο απευθείας API ως τελευταία
  λύση).

## v1.7.12
- **Seek σε παυσαρισμένο βίντεο δεν ξεκινάει πια play**: το double-tap seek (ή scrub bar)
  σε βίντεο που είναι ήδη σε παύση μετακινεί απλά τη θέση χωρίς να ξεκινάει αναπαραγωγή· σε
  βίντεο που ήδη παίζει, το seek συνεχίζει να επαναφέρει το play (όπως πριν). Το nudge
  (v1.7.6) έλεγχε μόνο "έπαιζε ποτέ πριν" — τώρα ξεχωρίζει σωστά πραγματική παύση από τον
  χρήστη (μετράει) από την προσωρινή παύση που προκαλεί το ίδιο το seek (αγνοείται).

## v1.7.13 (διαγνωστικό)
- Το auto-fullscreen σε rotation (v1.7.7/v1.7.10/v1.7.11) εξακολουθεί να μην ενεργοποιείται
  πάντα, παρά τα δύο προηγούμενα fixes. Αντί για τέταρτη μαντεψιά, προστέθηκε logging σε
  κάθε βήμα της λογικής (`MyTube[fsdebug]`, ίδιο μηχανισμό με το `[seekdebug]` — DebugLog +
  κουμπί "Αντιγραφή debug log" στις Ρυθμίσεις): αν καλείται καθόλου η function, αν βρίσκεται
  video, αν είναι paused/portrait, αν βρίσκεται το κουμπί fullscreen, τι συμβαίνει στο
  fallback. Σκοπός: να φανεί ΠΟΥ ακριβώς σταματάει η διαδικασία.

## v1.7.14
- **Βρέθηκε η πραγματική αιτία (χάρη στο fsdebug log)**: όταν το auto-fullscreen απέτυχε, το
  log έδειξε "no video found" σε **όλες** τις 8 προσπάθειες (~2") — το βίντεο απλά δεν είχε
  ακόμα φορτώσει (readyState=0, "waiting"), και στο ίδιο log φαίνεται ότι χρειάστηκε ~9.3"
  μέχρι να αποκτήσει πραγματικές διαστάσεις/να ξεκινήσει να παίζει. Όταν το βίντεο ήταν ήδη
  φορτωμένο, δούλεψε αμέσως στην πρώτη προσπάθεια. Fix: το retry παράθυρο μεγάλωσε από ~2"
  σε ~12" (48 προσπάθειες), αρκετό περιθώριο για πιο αργή φόρτωση χωρίς να τα παρατάει νωρίς.

## v1.7.15 (διαγνωστικό)
- Νέο log έδειξε ότι το κουμπί **βρίσκεται και γίνεται click** (attempt=1) ακόμα και σε
  περίπτωση που ο χρήστης ανέφερε ότι δεν μπήκε σε fullscreen — άρα είτε πιάνουμε λάθος
  στοιχείο (π.χ. κρυφό/desktop-only κουμπί που ταιριάζει τυχαία με το selector), είτε το
