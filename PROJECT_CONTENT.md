...
### 16. Seek σε παυσαρισμένο βίντεο δεν πρέπει να ξεκινάει play (v1.7.12)
Αίτημα του χρήστη: double-tap seek σε ήδη παυσαρισμένο βίντεο πρέπει να αλλάζει μόνο τη
θέση, όχι να ξεκινάει αναπαραγωγή· αν το βίντεο ήδη έπαιζε, το seek πρέπει να συνεχίσει να
παίζει μετά (η ήδη υπάρχουσα συμπεριφορά του nudge, ενότητα 6/v1.7.6).

- Το `mtShouldBePlaying` (nudge, ενότητα 6) μέχρι τώρα γινόταν `true` σε κάθε `playing` και
  **ποτέ δεν επανερχόταν σε `false`** — άρα μετά το πρώτο play ενός βίντεο, ένα seek σε
  παυσαρισμένη κατάσταση παρέμενε να "νομίζει" ότι έπρεπε να συνεχίσει να παίζει, κάνοντας
  nudge ακόμα κι όταν ο χρήστης το είχε παυσάρει επίτηδες.
- **Fix**: προστέθηκε `mtSeekInProgress` flag (true μεταξύ `seeking` και `seeked`) και ένας
  listener στο `pause` που μηδενίζει το `mtShouldBePlaying` **μόνο αν δεν είναι ήδη σε
...
### 19. Auto-fullscreen: το κουμπί "βρίσκεται" αλλά μάλλον είναι λάθος στοιχείο (v1.7.15)
Νέο log από τον χρήστη (σενάριο: βίντεο ήδη φορτωμένο) έδειξε `attempt=1 clicking
fullscreen button` — το selector βρήκε κάτι και έκανε click αμέσως, ΚΑΙ ΟΜΩΣ ο χρήστης
ανέφερε ότι δεν μπήκε σε fullscreen. Αυτό σημαίνει ότι το πρόβλημα των ενοτήτων 17-18
(retry timing) ήταν πραγματικό αλλά όχι το μοναδικό — υπάρχει και δεύτερη αιτία: πιθανόν το
`FULLSCREEN_BUTTON_SELECTOR` ταιριάζει με ένα στοιχείο που **δεν είναι** το πραγματικό
mobile fullscreen toggle (π.χ. ένα κρυφό desktop-skin κουμπί με ίδιο class name, ή κάτι με
παρόμοιο aria-label που κάνει κάτι άλλο).

- **Fix (μόνο diagnostic προς το παρόν)**: όταν βρίσκεται "κουμπί", καταγράφεται πλήρης
  περιγραφή του (`tagName`, `className`, `aria-label`, `title`, αν είναι ορατό
  `offsetParent !== null`, αν είναι `disabled`), και 300ms μετά το click ελέγχεται αν το
  `document.fullscreenElement` πράγματι άλλαξε.
- Επόμενο βήμα: νέο fsdebug log από τον χρήστη σε αποτυχημένη περίπτωση θα δείξει είτε (α)
...
### 22. Auto-fullscreen: disabled μετά από 8 αποτυχίες + νέα παλινδρόμηση (v1.7.18)
Ο χρήστης ανέφερε ότι, ακόμα και μετά το source=SOURCE_TOUCHSCREEN fix (ενότητα 21), το
auto-fullscreen εξακολουθεί να μην ενεργοποιείται· επιπλέον, όταν πατάει πρώτα μέσα στο
βίντεο πριν περιστρέψει, τώρα προκαλεί **επιπλέον ανεπιθύμητη παύση**. Αυτό δείχνει ότι το
synthetic touch μερικές φορές δεν πέφτει πάνω στο πραγματικό κουμπί fullscreen αλλά πάνω
στο ίδιο το βίντεο (τυχαία απόκλιση συντεταγμένων/timing), προκαλώντας ένα απλό
tap-toggle-play-pause αντί για fullscreen.

- **Απόφαση**: μετά από 8 συνολικά αποτυχημένες προσπάθειες (retry window, click στο
  πραγματικό κουμπί, native synthetic touch, source flag) και δεδομένου ότι η τελευταία
  προσπάθεια προκαλεί ενεργό πρόβλημα (όχι απλά αδράνεια), το auto-tap **απενεργοποιήθηκε**.
  Η function πλέον μόνο καταγράφει (`found button (auto-tap disabled): ...`) χωρίς να
  ενεργεί καθόλου — μηδενικός κίνδυνος side-effect, ο χρήστης πατάει μόνος του το κουμπί.
  Ο κώδικας του native synthetic-tap (`tapFullscreenButton`) παραμένει στο `MainActivity`
  σε περίπτωση που αξίζει να ξαναδοκιμαστεί με διαφορετική προσέγγιση (π.χ. dispatch μέσω
  `window.decorView` αντί απευθείας στο WebView — βλ. σημείωση ενότητας 21).

### 23. Διερεύνηση: "η εφαρμογή έκλεισε" κατά τη διάρκεια τηλεφωνικής κλήσης
Ο χρήστης ανέφερε ότι ενώ έβλεπε βίντεο σε fullscreen, δέχτηκε κλήση, και όταν τελείωσε η
κλήση η εφαρμογή είχε "κλείσει" εντελώς (όχι απλά επιστροφή σε προηγούμενη κατάσταση). Δύο
πιθανά σενάρια: (α) πραγματικό uncaught exception/crash, (β) το Android σκότωσε εντελώς τη
διαδικασία στο background λόγω μνήμης (πολύ συνηθισμένο σε Android WebView apps, ειδικά
κατά τη διάρκεια κλήσης που η ίδια η εφαρμογή τηλεφώνου χρειάζεται μνήμη) — στην περίπτωση
(β) δεν υπάρχει καν callback στον κώδικά μας, είναι αόρατο.

- **Global crash handler** (`MyTubeApp.installCrashHandler`): πιάνει οποιοδήποτε uncaught
  exception, γράφει το stack trace σε `Prefs.lastCrashInfo` (SharedPreferences — επιβιώνει
  ακόμα κι αν σκοτωθεί η ίδια η διαδικασία, σε αντίθεση με το in-memory `DebugLog`), μετά
  αφήνει τον default handler να συνεχίσει κανονικά (δεν αλλάζει τη συμπεριφορά crash, απλά
  προσθέτει durable καταγραφή). Στο επόμενο άνοιγμα της εφαρμογής
  (`surfaceLastCrashIfAny`), αν υπάρχει αποθηκευμένο crash, μπαίνει στο `DebugLog` με tag
  `MyTube[crash]` — εμφανίζεται στο ίδιο κουμπί "Αντιγραφή debug log" στις Ρυθμίσεις.
- **Debounce στο foreground service stop** (`MainActivity.setPlaybackServiceRunning`): πριν,
  κάθε πραγματική παύση σταματούσε αμέσως το `PlaybackService` — ακριβώς η στιγμή που η
  εφαρμογή πάει background (π.χ. λόγω κλήσης) είναι η πιο ευάλωτη σε τερματισμό από το
  σύστημα αν δεν υπάρχει καμία foreground προστασία. Τώρα η πραγματική διακοπή του service
  καθυστερεί ~45 δευτερόλεπτα (`Handler.postDelayed`, ακυρώνεται αν ξαναπαίξει το βίντεο
  πριν προλάβει) — αρκετό περιθώριο ώστε μια σύντομη κλήση να μην αφαιρέσει την προστασία
  ακριβώς τη στιγμή που χρειάζεται. Στο `onDestroy` η διακοπή γίνεται **αμέσως** (bypass του
  debounce), αφού εκεί η εφαρμογή πράγματι τερματίζεται.
- ⚠️ Αν το πρόβλημα ήταν σενάριο (β) (system kill χωρίς exception), το crash handler δεν θα
  δείξει τίποτα — σε αυτή την περίπτωση το debounce είναι η ουσιαστική διόρθωση. Επόμενο
  βήμα αν επιμείνει: ο χρήστης να ελέγξει το Debug log μετά το επόμενο τέτοιο περιστατικό.
